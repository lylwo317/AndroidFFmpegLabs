...
{
	Buf = (unsigned char*)malloc(H265_FRAME_MAX_LEN);
}
...
int h265Parser::h265_parser(unsigned char * buffer, unsigned int bufferlen, SH2645ParserInfo* pH2645ParserInfo) {
	if (NULL == buffer || bufferlen <= 0/* || nullptr == pH2645ParserInfo*/)
	{
		return -1;
	}
	h265_uinit();
	...
	bool bFindPPS = false;
#if SHOW_DETAIL
	LOGD("-----+-------- H265 NALU Table--+---------+\n");
	LOGD(" NUM |    POS  |    IDC |  TYPE |   LEN   |\n");
	LOGD("-----+---------+--------+-------+---------+\n");
#endif

	while(m_h265_buffer.length > 0 && !bFindPPS)
	{
		int data_lenth = 0;
		...
		switch(n->nal_unit_type){
			case H265_NAL_UNIT_VPS:
			{
				if (pH2645ParserInfo)
				{
					vps_pos = m_h265_buffer.readIdx - data_lenth;
					sprintf(type_str, "VPS");
					pH2645ParserInfo->AddVps(m_h265_buffer.data + (m_h265_buffer.readIdx - data_lenth), data_lenth);
				}
				break;
			}
			...
			case H265_NAL_UNIT_SPS: {
				if (pH2645ParserInfo)
				{
					pH2645ParserInfo->AddSps(m_h265_buffer.data + (m_h265_buffer.readIdx - data_lenth), data_lenth);
					h265_parser_sps(m_h265_buffer.data + (m_h265_buffer.readIdx - data_lenth + m_start_code_size), (data_lenth - m_start_code_size)
						, pH2645ParserInfo->m_iWidth, pH2645ParserInfo->m_iHeight, pH2645ParserInfo->m_iYUVFormat);
				}
				m_isSps = 1;
				break;
			}
			case H265_NAL_UNIT_PPS:sprintf(type_str, "PPS");
			{
				if (pH2645ParserInfo)
				{
					pH2645ParserInfo->AddPps(m_h265_buffer.data + (m_h265_buffer.readIdx - data_lenth), data_lenth);
					pH2645ParserInfo->AddExtraData(m_h265_buffer.data + vps_pos, m_h265_buffer.readIdx - vps_pos);
				}
				bFindPPS = true;
				break;
			}
		...
		switch(n->nal_reference_idc>>5){
			case NALU_PRIORITY_DISPOSABLE:sprintf(type_str, "DISPOS");break;
			case NALU_PRIRITY_LOW:sprintf(type_str, "LOW");break;
			case NALU_PRIORITY_HIGH:sprintf(type_str, "HIGH");break;
			case NALU_PRIORITY_HIGHEST:sprintf(type_str, "HIGHEST");break;
		}
#if SHOW_DETAIL
		LOGD("%5d| %8d| %6s| %15s| %8d|\n",nal_num,data_offset,idc_str,type_str,n->len);
#endif
...
int h265Parser::h265_parse_nalu(unsigned char * buffer, unsigned int bufferlen, std::list<NALU_t*>& listNalu)
{
...
int h265Parser::h265_GetAnnexbNALU (NALU_t *nalu){
	...
	while (!StartCodeFound){
		if (0 == m_h265_buffer.length){
			...
			//nalu->buf = (char*)malloc(nalu->len);
			memcpy (nalu->buf, &Buf[nalu->startcodeprefix_len - nalu->startcodeprefix_len], nalu->len);
			nalu->forbidden_bit = nalu->buf[0+ nalu->startcodeprefix_len] & 0x80; //1 bit
			nalu->nal_reference_idc = nalu->buf[0 + nalu->startcodeprefix_len] & 0x60; // 2 bit
			nalu->nal_unit_type = (nalu->buf[0 + nalu->startcodeprefix_len]) & 0x1f;// 5 bit
			return pos-1;
		}
	...
	nalu->forbidden_bit = nalu->buf[0 + nalu->startcodeprefix_len] & 0x80; //1 bit
	nalu->nal_reference_idc = nalu->buf[0 + nalu->startcodeprefix_len] & 0x60; // 2 bit
	nalu->nal_unit_type = (nalu->buf[0 + nalu->startcodeprefix_len]& 0x7E)>>1;// & 0x7E)>>1;
...
int h265Parser::h265_parser_sps(unsigned char * buffer, unsigned int bufferlen, int& iWidth, int& iHeight, int& iFormat){
   ...
   de_emulation_prevention(buffer,&bufferlen);
	...
	iWidth = pic_width_in_luma_samples;
	iHeight = pic_height_in_luma_samples;
