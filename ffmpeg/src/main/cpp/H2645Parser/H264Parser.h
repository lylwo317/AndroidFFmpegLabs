...
public:
	int h264_parser(unsigned char * buffer, unsigned int bufferlen, SH2645ParserInfo* pH2645ParserInfo);
	HEVCParser::stHDRMetadata h264info;
	int m_isSps;
private:
	int h264_parser_sps(unsigned char * buffer, unsigned int bufferlen, int& iWidth, int& iHeight, int& iFormat);
