...
#define H265_FRAME_MAX_LEN (1024*1024)
...
public:
	int h265_parser(unsigned char * buffer, unsigned int bufferlen, SH2645ParserInfo* pH2645ParserInfo);
	int h265_parse_nalu(unsigned char * buffer, unsigned int bufferlen, std::list<NALU_t*>& listNalu);
	void h265_init(unsigned char * buffer, unsigned int bufferlen);
	void h265_uinit();
	HEVCParser::stHDRMetadata m_h265info;
	int m_isSps;
private:
	int h265_parser_sps(unsigned char * buffer, unsigned int bufferlen, int& iWidth, int& iHeight, int& iFormat);
