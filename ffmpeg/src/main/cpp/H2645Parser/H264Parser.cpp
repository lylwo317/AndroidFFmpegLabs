...
int h264Parser::h264_parser(unsigned char * buffer, unsigned int bufferlen, SH2645ParserInfo* pH2645ParserInfo)
{
	...
	while(m_h264_buffer.length > 0 )
	{
		...
		switch(n->nal_unit_type){
			...
			case NALU_TYPE_SPS:sprintf(type_str, "SPS");
				...
				if (pH2645ParserInfo)
				{
					sps_pos = m_h264_buffer.readIdx - data_lenth;
					pH2645ParserInfo->AddSps(m_h264_buffer.data + (m_h264_buffer.readIdx - data_lenth), data_lenth);
					h264_parser_sps(m_h264_buffer.data + (m_h264_buffer.readIdx - data_lenth + m_start_code_size), (data_lenth - m_start_code_size), pH2645ParserInfo->m_iWidth, pH2645ParserInfo->m_iHeight, pH2645ParserInfo->m_iYUVFormat);
				}
                m_isSps = 1;
                bFindSPS = true;
				break;
			...
			case NALU_TYPE_PPS:sprintf(type_str, "PPS");
			{
				if (pH2645ParserInfo)
				{
					pH2645ParserInfo->AddPps(m_h264_buffer.data + (m_h264_buffer.readIdx - data_lenth), data_lenth);
					pH2645ParserInfo->AddExtraData(m_h264_buffer.data + sps_pos, m_h264_buffer.readIdx - sps_pos);
				}
				bFindPPS = true;
				break;
			}
...
int h264Parser::h264_parser_sps(unsigned char * buf, unsigned int nLen, int& iWidth, int& iHeight, int& iFormat){
	int width = 0;
	int height = 0;
	double fps = 0;
    ...
    if(nal_unit_type==7)
    {
        ...
        if(frame_cropping_flag)
        {
			...
			int _iWidth = ((pic_width_in_mbs_minus1 + 1) * 16) - frame_crop_right_offset * 2 - frame_crop_left_offset * 2;
			int _iHeight = ((2 - frame_mbs_only_flag)* (pic_height_in_map_units_minus1 + 1) * 16) - (frame_crop_bottom_offset * 2) - (frame_crop_top_offset * 2);
			iWidth = _iWidth;
			iHeight = _iHeight;
        }
